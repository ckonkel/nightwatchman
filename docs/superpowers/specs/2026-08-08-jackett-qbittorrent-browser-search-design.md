...
- A bounded, secret-free HTTP readiness check against Jackett on localhost.

Gluetun publishes required `${JACKETT_PORT}` TCP traffic. Jackett declares no ports of its own because Compose forbids port publishing with service network mode.
...
The deployable LinuxServer image tag or digest will be recorded during the predeployment read-only discovery step. No floating or invented version will be silently substituted for the approved value.
...
- Confirm the Web UI exposes the Search tab and can download a search result.
...
If the current qBittorrent Web UI cannot perform the browser workflow, deployment stops. A qBittorrent upgrade is a separate decision requiring an image/version review and explicit approval; it is not bundled into this change.
...
4. Record the current Gluetun and qBittorrent image references and rendered, sanitized stack definition.
...
Rollback removes or disables the qBittorrent Jackett plugin, removes the Jackett service and Gluetun's 9117 publication from Compose, and redeploys the previously recorded image references. The Jackett config directory is retained until rollback is verified. The existing qBittorrent config/download binds are never deleted or replaced. If plugin installation altered qBittorrent state unexpectedly, restore the verified predeployment qBittorrent config backup while qBittorrent is stopped.
